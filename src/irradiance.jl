...
function angle_of_incidence_projection(
    ...
    return cosd(surface_roll) * cosd(surface_tilt) * sind(solpos.apparent_elevation) +
           cosd(solpos.apparent_elevation) *
           sind(solpos.azimuth) *
           (
...
function beam_component(
    ...
    aoi_proj =
        angle_of_incidence_projection(solpos, surface_tilt, surface_roll, surface_azimuth)
...
function sky_diffuse_component_haydavies(
    ...
    poa_isotropic =
        dhi * (1 - anisotropy_index) * 0.5(1 + cosd(surface_tilt) * cosd(surface_roll))
...
function sky_diffuse_component_king(
    ...
    sky_diffuse =
        0.5dhi * (1.0 + cosd(surface_tilt) * cosd(surface_roll)) +
        0.5ghi *
        (0.012 * (90.0 - solpos.elevation - 0.04)) *
...
function total_irradiance(
    ...
    surface_azimuth;
    albedo = ALBEDO.urban,
    sky_diffuse_component = sky_diffuse_component_isotropic(
        irrad.dhi,
        surface_tilt,
        surface_roll,
    ),
